...
$open_file = fopen('C:\wamp\www\acronyms\mili_gov.txt', 'r');
$open_file2 = fopen('C:\wamp\www\acronyms\mili_gov_queries.txt', 'a');
...
{	
	...
	$newline = "Insert into acronymlist VALUES ('". trim($split_line[0]) . "','" . trim($split_line[1]) . "','Mili-Gov', 1);" .PHP_EOL;
	fwrite($open_file2, $newline);
}

fclose($open_file);

/*
	$dup_lines = file('C:\wamp\www\acronyms\IT.txt');
	$dup_lines = array_unique($dup_lines);

	file_put_contents('C:\wamp\www\acronyms\IT.txt', implode($dup_lines));
*/
