...
$open_file = fopen('C:\wamp\www\acronyms\mili_gov.txt', 'r'); // Open the mili_gov.txt file where the acronyms and definitions are contained. 
$open_file2 = fopen('C:\wamp\www\acronyms\mili_gov_queries.txt', 'a'); // Open another file which will store the queries. 

// loop through the file containing acronyms and definitions line by line. 
while(!feof($open_file))
{	
	$line = fgets($open_file); // get the line
	$split_line = explode(" = ", $line); // split the line into an array, where " = " occurs

	$newline = "Insert into acronymlist VALUES ('". trim($split_line[0]) . "','" . trim($split_line[1]) . "','Mili-Gov', 1);" .PHP_EOL; // Create the string for the query. Trim removes the whitespace characters as without trim it was returning extra whitespace characters. Change the 'Mili_Gov' in this string to be appropriate category.  
